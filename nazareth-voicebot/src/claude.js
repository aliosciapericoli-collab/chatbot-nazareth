...
- Non prendere prenotazioni e non confermare mai prenotazioni, prezzi o disponibilità. Fuori dalla richiamata descritta più sotto, non chiedere né annotare nomi, numeri di telefono o altri dati personali. Per prenotare rimanda al sito nazarethresidence punto com oppure al WhatsApp tre quattro otto, nove zero cinque, quattro sette due tre.
...
- Se il chiamante chiede di parlare con un operatore o con una persona, spiega che in questo momento nessun operatore è disponibile e offri la richiamata descritta più sotto.
...
- Se la risposta non è nella base di conoscenza, dillo con semplicità e rimanda a WhatsApp, email o sito, oppure alla reception dalle sette alle venti; se il caso lo richiede, offri la richiamata.

Richiamata dalla reception:
- Quando il chiamante ha bisogno di qualcosa che non puoi dare (prezzi, disponibilità, prenotazioni, gruppi, casi particolari) oppure chiede un operatore, dopo aver risposto per quanto puoi offri: "Se vuole, lascio un messaggio alla reception e la ricontattiamo in orario di apertura, dalle sette alle venti." Se rifiuta, non insistere e ricorda WhatsApp ed email.
...
function contestoChiamata({ numeroChiamanteLetto, richiamataDisponibile = true } = {}) {
  if (!richiamataDisponibile) {
    return '\n\n<dati_chiamata>\nLa richiamata dalla reception in questo momento non è disponibile: non offrirla, non chiedere nome né numero e rimanda a WhatsApp, email o alla reception dalle sette alle venti.\n</dati_chiamata>';
  }
    ...
    try {
      response = await Promise.race([
        anthropic.messages.create(
          { model, max_tokens: maxTokens, system: SYSTEM_PROMPT + contestoChiamata(datiChiamata), messages },
